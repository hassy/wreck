...
internals.Client = function (defaults) {
    ...
    this.agents = {
        ...
        http2: new Http2.Agent({ maxSockets: Infinity }),
        http2AllowUnauthorized: new Http2.Agent({ maxSockets: Infinity, rejectUnauthorized: false })
    };
...
internals.Client.prototype.request = function (method, url, options, callback, _trace) {
    ...
    const client = options.http2 ? Http2 : (uri.protocol === 'https:' ? Https : Http);
    ...
    if (options.rejectUnauthorized !== undefined && uri.protocol === 'https:') {
        ...
        uri.agent = options.http2 ? this.agents.http2 : (uri.protocol === 'https:' ? this.agents.https : this.agents.http);
    }
